...
async function updateForecastData(lat, lon) {
    try {
        ...
        // Calculate average of all user-reported temperatures
        const avgUserTemp = userReportedTemps.length > 0 
            ? userReportedTemps.reduce((sum, temp) => sum + temp.temperature, 0) / userReportedTemps.length 
            : null;

        // Update comparison container values
        document.getElementById('avgUserTemp').textContent = avgUserTemp ? `${avgUserTemp.toFixed(1)}°C` : '--°C';
        document.getElementById('totalReports').textContent = userReportedTemps.length;
