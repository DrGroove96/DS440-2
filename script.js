...
function initializeCharts() {
    // Register the boxplot and violin plot controllers
    try {
        if (typeof Chart.BoxPlot !== 'undefined') {
            Chart.register(Chart.BoxPlot);
            console.log('BoxPlot plugin registered successfully');
        } else {
            console.warn('BoxPlot plugin not found');
        }
        
        if (typeof Chart.ViolinPlot !== 'undefined') {
            Chart.register(Chart.ViolinPlot);
            console.log('ViolinPlot plugin registered successfully');
        } else {
            console.warn('ViolinPlot plugin not found');
        }
    } catch (error) {
    ...
    if (tempCtx) {
        hourlyTempChart = new Chart(tempCtx.getContext('2d'), {
            type: 'line',
            data: {
                labels: [],
                datasets: [
    ...
    const dailyTempCtx = document.getElementById('dailyTemperatureChart');
    if (dailyTempCtx) {
        dailyTempChart = new Chart(dailyTempCtx.getContext('2d'), {
            type: 'line',
            data: {
                labels: [],
                datasets: [{
    ...
    if (precipCtx) {
        precipitationChart = new Chart(precipCtx.getContext('2d'), {
            type: 'bar',
            data: {
                labels: [],
                datasets: [
    ...
    const trendsCtx = document.getElementById('temperatureTrendsChart');
    if (trendsCtx) {
        temperatureTrendsChart = new Chart(trendsCtx.getContext('2d'), {
            type: 'line',
            data: {
                labels: [],
                datasets: [
    ...
    if (hourlyDistributionCtx) {
        hourlyDistributionChart = new Chart(hourlyDistributionCtx.getContext('2d'), {
            type: 'bar',
            data: {
                labels: [],
                datasets: [
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Hourly Temperature Distribution'
                    }
                }
            }
        });
    ...
    try {
        ...
        if (!comparisonContainer) {
            ...
            comparisonContainer.innerHTML = `
                ...
                <div class="comparison-item">
                    <div class="comparison-label">Today's Reports</div>
                    <div class="comparison-value" id="todayUserTempCount">--</div>
                </div>
                <div class="comparison-item">
                    <div class="comparison-label">Latest Report</div>
                    <div class="comparison-value" id="latestUserTemp">--°C</div>
                </div>
            `;
        ...
        // Separate today's user-reported temperatures from historical data
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        
        const todayUserTemps = userReportedTemps.filter(temp => {
            const reportDate = new Date(temp.timestamp);
            reportDate.setHours(0, 0, 0, 0);
            return reportDate.getTime() === today.getTime();
        });
        
        const historicalUserTemps = userReportedTemps.filter(temp => {
            const reportDate = new Date(temp.timestamp);
            reportDate.setHours(0, 0, 0, 0);
            return reportDate.getTime() < today.getTime();
        });
        ...
        createHistoricalAnalysis(historicalUserTemps);
        updateHistoricalChart();

        // Create arrays for today's user-reported temperatures
        const userTempsData = [];
        const userTempsLabels = [];
        
        // Add each user-reported temperature with its exact time
        todayUserTemps.forEach(temp => {
            userTempsData.push(temp.temperature);
            userTempsLabels.push(formatTime(temp.timestamp.getTime() / 1000));
        });

        // Update the comparison container values
        document.getElementById('todayUserTempCount').textContent = todayUserTemps.length;
        if (userReportedTemps.length > 0) {
            const latestTemp = userReportedTemps[userReportedTemps.length - 1];
            document.getElementById('latestUserTemp').textContent = 
                `${latestTemp.temperature.toFixed(1)}°C (${formatTime(new Date(latestTemp.timestamp).getTime() / 1000)})`;
        }
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (context.parsed.y !== null) {
                                    ...
                                    // Add special message for user-reported temperatures
                                    if (context.dataset.label === "Today's User Reports (°C)") {
                                        const timeLabel = userTempsLabels[context.dataIndex];
                                        if (timeLabel) {
                                            label += ` (Reported at ${timeLabel})`;
                                        }
                                    }
                                }
                                return label;
        ...
        // Group user-reported temperatures by date and calculate averages
        const userTempsByDate = {};
        todayUserTemps.forEach(temp => {
            const date = new Date(temp.timestamp);
            const dateStr = formatDate(date.getTime() / 1000);
            if (!userTempsByDate[dateStr]) {
                userTempsByDate[dateStr] = {
                    temps: [],
                    latest: null
                };
            }
            userTempsByDate[dateStr].temps.push(temp.temperature);
            if (!userTempsByDate[dateStr].latest || temp.timestamp > userTempsByDate[dateStr].latest.timestamp) {
                userTempsByDate[dateStr].latest = {
        ...
        // Create arrays for daily user-reported temperatures
        const dailyUserTempsData = [];
        const dailyUserTempsLabels = [];
        const dailyUserTempsAverages = [];
        
        // Add average and latest temperatures for each day
        dailyLabels.forEach(dateStr => {
            if (userTempsByDate[dateStr]) {
                const dayData = userTempsByDate[dateStr];
                // Calculate average
                const avg = dayData.temps.reduce((sum, temp) => sum + temp, 0) / dayData.temps.length;
                dailyUserTempsAverages.push(avg);
                
                // Add latest temperature
                if (dayData.latest) {
                    dailyUserTempsData.push(dayData.latest.temperature);
                    dailyUserTempsLabels.push(`${dateStr} ${formatTime(dayData.latest.timestamp.getTime() / 1000)}`);
                } else {
                    dailyUserTempsData.push(null);
                    dailyUserTempsLabels.push(null);
                }
            } else {
                dailyUserTempsAverages.push(null);
                dailyUserTempsData.push(null);
                dailyUserTempsLabels.push(null);
            }
        });
        ...
        dailyTempChart = new Chart(dailyCtx, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    {
                        label: "User Reports Average (°C)",
                        data: dailyUserTempsAverages,
                    ...
                    {
                        label: "Latest User Report (°C)",
                        data: dailyUserTempsData,
                        borderColor: 'rgb(54, 162, 235)',
                        backgroundColor: 'rgba(54, 162, 235, 0.8)',
                        tension: 0,
                        borderWidth: 3,
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (context.parsed.y !== null) {
                                    ...
                                    // Add special messages for user-reported temperatures
                                    if (context.dataset.label === "Latest User Report (°C)") {
                                        const timeLabel = dailyUserTempsLabels[context.dataIndex];
                                        if (timeLabel) {
                                            label += ` (Reported on ${timeLabel})`;
                                        }
                                    } else if (context.dataset.label === "User Reports Average (°C)") {
                                        const dateStr = context.label;
                                        if (userTempsByDate[dateStr]) {
                                            const count = userTempsByDate[dateStr].temps.length;
        ...
        updateTemperatureTrendsChart(userReportedTemps);
...
function updateTemperatureTrendsChart(temperatures) {
    ...
    sortedTemps.forEach(temp => {
        ...
        tempsByDate[date].push(temp.temperature);
    });
    
    // Calculate daily average temperatures
    const dates = Object.keys(tempsByDate).sort();
    const dailyTemps = dates.map(date => {
    ...
    for (let i = 0; i < dailyTemps.length; i++) {
        if (i < 6) {
            // Not enough data for a 7-day average yet
            movingAverage.push(null);
        } else {
            // Calculate average of last 7 days
            const sum = dailyTemps.slice(i-6, i+1).reduce((a, b) => a + b, 0);
            movingAverage.push(sum / 7);
        }
    }
    
    // Calculate temperature trend (linear regression)
    const trendLine = calculateTrendLine(dates.map((_, i) => ({ x: i, y: dailyTemps[i] })));
    const trendData = dates.map((_, i) => trendLine.start + (trendLine.slope * i));
    
    // Update chart data
    temperatureTrendsChart.data.labels = dates;
    temperatureTrendsChart.data.datasets[0].data = dailyTemps;
    temperatureTrendsChart.data.datasets[1].data = movingAverage;
    temperatureTrendsChart.data.datasets[2].data = trendData;
